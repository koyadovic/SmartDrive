 ...
 */
public class SDPath implements Path {

    private final SDFileSystem filesystem;
    private final byte[] path;
    private volatile int[] offsets;
    private volatile int hash = 0;
    private volatile byte[] resolved = null;

    public SDPath(SDFileSystem filesystem, byte[] path){
        this(filesystem, path, false);
    }

    public SDPath(SDFileSystem filesystem, byte[] path, boolean normalized){
    ...
    private static byte[] doGetResolved(SDPath p) {
    ...
    private static boolean equalsNameAt(SDPath p1, SDPath p2, int index) {
    ...
    @Override
    public SDFileSystem getFileSystem() {
        return filesystem;
    }
    ...
    @Override
    public SDPath getRoot() {
        if (isAbsolute()) {
            return new SDPath(filesystem, new byte[] { this.path[0] });
        }
        return null;
    }

    @Override
    public SDPath getFileName() {
        initOffsets();
        ...
        return new SDPath(filesystem, path);
    }

    @Override
    public SDPath getParent() {
        initOffsets();
        int nbOffsets = offsets.length;
        if (nbOffsets == 0) {
            return null;
    ...
    @Override
    public SDPath getName(int index) {
        initOffsets();
    ...
    @Override
    public SDPath subpath(int beginIndex, int endIndex) {
        initOffsets();
    ...
    @Override
    public SDPath normalize() {
        byte[] p = getResolved();
        if (p == this.path) {
            return this;
        }
        return new SDPath(filesystem, p, true);
    }

    @Override
    public SDPath resolve(Path other) {
        SDPath p1 = this;
        SDPath p2 = checkPath(other);
        if (p2.isAbsolute()) {
            return p2;
        ...
        return new SDPath(filesystem, result);
    }

    @Override
    public SDPath resolve(String other) {
    ...
    @Override
    public SDPath relativize(Path other) {
        SDPath p1 = this;
        SDPath p2 = checkPath(other);
        if (p2.equals(p1)) {
            return new SDPath(filesystem, new byte[0], true);
        }
    ...
    public SDPath toAbsolutePath() {
        ...
        return new SDPath(filesystem, result, true);
    }

    @Override
    public SDPath toRealPath(LinkOption... options) throws IOException {
        SDPath absolute = new SDPath(filesystem, getResolvedPath()).toAbsolutePath();
    ...
    public boolean equals(Object other) {
        return other instanceof SDPath
                && ((SDPath) other).filesystem == filesystem
                && compareTo((SDPath) other) == 0;
    }
    ...
    private SDPath checkPath(Path paramPath) {
        if (paramPath == null) {
            throw new NullPointerException();
        }
        if (!(paramPath instanceof SDPath)) {
            throw new ProviderMismatchException();
        }
        return (SDPath) paramPath;
    }
